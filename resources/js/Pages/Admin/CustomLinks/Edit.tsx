...
export default function Edit({ customLink }: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        slug: customLink.slug,
        destination_url: customLink.destination_url,
        label: customLink.label ?? '',
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('admin.custom-links.update', customLink.id));
    };

    return (
        <AdminLayout>
            <Head title="Edit Custom Link" />
            ...
            <h1 className="text-2xl font-bold text-white mb-6">Edit Custom Link</h1>
