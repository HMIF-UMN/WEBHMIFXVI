...
function SubmissionRow({ sub }: { sub: Submission }) {
    ...
    const handleExpand = () => {
        setExpanded((v) => !v);
        if (!isRead) {
            router.patch(route('admin.contact-submissions.read', sub.id), {}, { preserveScroll: true });
        }
    };

    const handleDelete = () => {
        if (!confirm('Delete this submission?')) return;
        router.delete(route('admin.contact-submissions.destroy', sub.id), { preserveScroll: true });
    };
