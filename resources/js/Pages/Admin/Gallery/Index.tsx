...
function UploadForm() {
    ...
    return (
        <form onSubmit={submit} className="bg-[#1a1a1a] border border-gray-800 rounded-lg p-5 mb-6 flex flex-wrap gap-4 items-end">
            <div>
            ...
            <div>
                ...
                <input
                    ...
                    className="bg-[#111] border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500 w-48"
                />
            </div>
            ...
            <div>
                ...
                <input
                    ...
                    className="bg-[#111] border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-amber-500 w-20"
                />
            </div>
            ...
            <button
                ...
                className="px-4 py-1.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white text-sm font-medium rounded transition-colors"
            >
...
function EditModal({ image, onClose }: { image: GalleryImage; onClose: () => void }) {
    const { data, setData, post, processing, errors } = useForm<{
        ...
        image: File | null;
        _method: string;
    }>({ title: image.title ?? '', order: String(image.order), image: null, _method: 'PATCH' });
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-1">
                        <button
                            type="submit"
                            disabled={processing}
                            className="px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white text-sm font-medium rounded transition-colors"
                        >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                    {images.map((img) => (
                        <div key={img.id} className="group relative bg-[#1a1a1a] border border-gray-800 rounded-lg overflow-hidden">
                            ...
                            <div className="px-2 pb-1.5 text-xs text-gray-600">order: {img.order}</div>
                        </div>
                    ))}
                </div>
            )}
