    ...
    return (
        <div onClick={onClick} className={`font-kanit h-auto flex flex-col sm:flex-row gap-3 sm:gap-4 py-3 sm:py-4 px-0 ${!isLast ? 'border-b border-slate-700/20' : ''} ${onClick ? 'cursor-pointer hover:opacity-80' : ''} transition-opacity duration-200`}>
            <div className="flex-1 min-w-0">
                <span className="inline-block px-2.5 py-1 mb-2 text-xs font-black uppercase tracking-widest text-slate-900 rounded-full" style={{ backgroundColor: getCategoryColor(category) }}>{getCategoryLabel(category)}</span>
                <h4 className="text-sm sm:text-base font-black text-slate-100 mb-1 line-clamp-2">{title}</h4>
