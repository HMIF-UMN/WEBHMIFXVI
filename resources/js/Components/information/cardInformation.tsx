            ...
            <div className="p-4 md:p-5 lg:p-6">
                <div className="flex flex-wrap gap-2 mb-4">
                    {categoryList.map((cat, i) => (
                        <span key={i} className="inline-block px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest text-slate-900" style={{ backgroundColor: getCategoryColor(cat) }}>{getCategoryLabel(cat)}</span>
                    ))}
                </div>
                <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-black text-slate-100 mb-2 md:mb-3 line-clamp-3 leading-tight tracking-tight">{title}</h3>
